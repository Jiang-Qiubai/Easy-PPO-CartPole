    ...
    while episode < agent.config.max_episodes:
        ...
        while not done:
            for t in range(agent.config.T_horizon):
                ...
                m = Categorical(prob)
                # ​输入​：prob是神经网络的输出（动作概率向量）。
                # ​输出​：一个分布对象 m，可以调用以下方法：
                # m.sample()：按概率采样动作。
                # m.log_prob(action)：计算动作的对数概率（用于策略梯度计算）。
